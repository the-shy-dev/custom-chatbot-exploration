...
class Chatbot:
    def __init__(self, db_folder: str = "./vectorstore"):
        self.embedding = OpenAIEmbeddings(model="text-embedding-ada-002") # changed to make it faster

        if not os.path.exists(db_folder) or len(os.listdir(db_folder)) == 0:
            raise ValueError("No precomputed VectorDB found! Run `python prepare_vectordb.py` first.")
